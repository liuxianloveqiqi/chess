...
func (m *JWTMiddleware) Handle(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if authHeader == "" {
			...
			err, _ := json.Marshal("The auth in the request header is empty")
			w.Write(err)
			return
		}
		fmt.Println(22222)
		...
		if !(len(parts) == 3 && parts[0] == "Bearer") {
			w.WriteHeader(http.StatusBadRequest)
			err, _ := json.Marshal("The auth in the request header is incorrectly formatted")
			w.Write(err)
			return
		}
		...
		if err != nil {
			...
			err, _ := json.Marshal("token authentication failed")
			w.Write(err)
			return
		}
		if isExpire {
